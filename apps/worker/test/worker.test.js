...
import { mapErrorToIncident, processGithubWebhookJob, processLogEventForIncident, queueCatalog, runRemediation } from "../src/index.js";
...
describe("log incident pipeline", () => {
    it("log event with level=error creates incident candidate with deterministic fingerprint", () => {
        const empty = { lastSeenByFingerprint: new Map() };
        const a = processLogEventForIncident({ tenantId: "t1", logEvent: baseLogEvent({ message: "PID=1 oops" }), cooldownMs: 60_000 }, empty);
        const b = processLogEventForIncident({ tenantId: "t1", logEvent: baseLogEvent({ message: "PID=2 oops" }), cooldownMs: 60_000 }, empty);
        ...
        expect(a.incident.fingerprint).toBe(mapErrorToIncident({ message: "PID=1 oops", tenantId: "t1", serviceId: "svc-1" }).fingerprint);
    });
    it("log event with non-error level is ignored", () => {
        const r = processLogEventForIncident({ tenantId: "t1", logEvent: baseLogEvent({ level: "info" }), cooldownMs: 60_000 }, { lastSeenByFingerprint: new Map() });
...
describe("github webhook job processor", () => {
    it("handles supported mutation actions", () => {
        const create = processGithubWebhookJob({
        ...
        const merge = processGithubWebhookJob({
            kind: "github_mutation",
            tenantId: "t-1",
            installationId: 1,
            action: "merge_pr",
        ...
        const push = processGithubWebhookJob({
            kind: "github_mutation",
            tenantId: "t-1",
            installationId: 1,
            action: "push",
    ...
    it("returns ingestion success for ingestion placeholder jobs", () => {
        const r = processGithubWebhookJob({
    ...
    it("returns failure for unsupported mutation action", () => {
        const r = processGithubWebhookJob({
            kind: "github_mutation",
            tenantId: "t-1",
            installationId: 1,
    ...
    it("returns failure for invalid payload", () => {
        const r = processGithubWebhookJob({ not: "valid" });
        expect(r.ok).toBe(false);
        if (r.ok)
            throw new Error("unreachable");
